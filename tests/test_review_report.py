...
class ReviewReportTests(unittest.TestCase):
    ...
    def test_apply_review_marking_sets_reviewed_flag(self) -> None:
        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            index_path = root / "metadata_index.sqlite"
            index = SQLiteMetadataIndex(index_path)
            ...
            apply_review_marking(index_path, load_records(index_path), reviewed=True, scope="needs_review", dry_run=False, write_every=1, export_json=False)
